...
$JSON="{ `"options`": { `"max_versions`": 12 }, `"data`": { `"password`": `"$NEWPASS`", `"username`": `"$USERNAME`" } }"

# First commit the new password to vault, then change it locally.
Invoke-RestMethod -Headers @{"X-Vault-Token" = ${VAULT_TOKEN}} -Method POST -Body $JSON -Uri ${VAULT_ADDR}/v1/instance-creds/windows/${HOSTNAME}/$($USERNAME.ToLower())
if($?) {
